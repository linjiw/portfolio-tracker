...
const VALID_SEG={ov:['score','decide','aisemi','qt','nw','risk','struct','cmp','pfib','sig','beh','journal','rebal'], stk:['price','tx','fib','journal']};   // allowlist: keeps invalid #/stock/NVDA/banana from silently overwriting localStorage and lets us reject typo-URLs
const SEG_LABEL={score:'决策一览', decide:'决策分析', aisemi:'AI半导体', qt:'QQQ/TQQQ', nw:'净值·账户', risk:'波动贡献', struct:'结构', cmp:'指数对比', pfib:'技术·节奏', sig:'持仓信号', beh:'行为决策', rebal:'再平衡计划', price:'价格 · 操作', tx:'交易明细', fib:'斐波那契'};
...
function aiSemiQuantCard(){
 ...
 const deltaCount=deltaCoverage.attributed??deltaMeta.attributedCount??0,deltaTotal=deltaCoverage.total??deltaMeta.totalCount??0;
 const cards=[
  ...
  ['Score Δ',`${deltaCount}/${deltaTotal}`,'vs previous ai_semi_quant.json snapshot'],
...
function renderOverview(){
 ...
 const segs=['score','decide','aisemi','qt','nw','risk','struct','cmp','pfib','sig','beh','journal','rebal'];
...
function onboardStrip(){let done=false;try{done=localStorage.getItem('ptrak.onboard.v1')==='done';}catch(e){}if(done)return'';
 ...
 return `<div id="onboard" class="note" style="margin-bottom:14px;padding:10px 13px;background:var(--panel);border:1px solid var(--line);border-radius:var(--r-card);line-height:1.7">
   <b style="color:var(--accent)">从这里开始</b> · 想知道我赚了多少 → ${j('score','决策一览')} / ${j('nw','净值')}　|　AI/半导体框架 → ${j('aisemi','AI半导体')}　|　QQQ/TQQQ 节奏 → ${j('qt','策略台')}　|　钱和风险集中在哪 → ${j('struct','结构')} / ${j('risk','风险')}　|　该不该调仓 → ${j('beh','行为决策')} / ${j('rebal','再平衡计划')}
