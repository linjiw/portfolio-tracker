...
class OptionSnapshotStore:
    ...
    def __init__(self, rows, max_age_days=0, strike_tolerance=0.01, max_age_minutes=None):
        ...
        for raw in rows:
            ...
            if not snapshot_ts or not expiry or option_type not in ("call", "put") or strike is None:
                continue
            row = dict(raw)
            row["snapshot_ts"] = snapshot_ts
            row["snapshot_date"] = mmb.date_key(snapshot_ts)
            row["expiry"] = expiry
            row["option_type"] = option_type
            row["strike"] = strike
            row["_snapshot_dt"] = parse_snapshot_ts(snapshot_ts)
            for key in (
    ...
    def from_path(cls, path, max_age_days=0, strike_tolerance=0.01, max_age_minutes=None):
        ...
        return cls(rows, max_age_days=max_age_days, strike_tolerance=strike_tolerance, max_age_minutes=max_age_minutes)
    ...
    def chain_rows(self, entry_date, expiry, timestamp_policy="same_day_close", max_age_minutes=None):
        ...
        for age in range(self.max_age_days + 1):
            key_date = (entry - dt.timedelta(days=age)).isoformat()
        ...
        for row in candidates:
            ...
            if max_minutes is not None:
                ...
                if age_minutes < 0 or age_minutes > float(max_minutes):
                    continue
            valid.append(row)
        if not valid:
            return [], "stale_snapshot"
        latest_ts = max(row["snapshot_ts"] for row in valid)
        return [row for row in valid if row["snapshot_ts"] == latest_ts], None
...
def black_scholes_price(spot, strike, years, sigma, option_type, rate=0.04):
...
def black_scholes_delta(spot, strike, years, sigma, option_type, rate=0.04):
    spot = max(float(spot), 1e-9)
    strike = max(float(strike), 1e-9)
    years = max(float(years), 1.0 / 365.0)
    sigma = max(float(sigma), 0.01)
...
def spread_mark(spot, short_strike, long_strike, side, years, sigma, rate=0.04):
    if side == "CCS":
        short_value = black_scholes_price(spot, short_strike, years, sigma, "call", rate)
        long_value = black_scholes_price(spot, long_strike, years, sigma, "call", rate)
    elif side == "PCS":
        short_value = black_scholes_price(spot, short_strike, years, sigma, "put", rate)
        long_value = black_scholes_price(spot, long_strike, years, sigma, "put", rate)
    else:
...
def expiry_index(rows, entry_idx, hold_bars):
    exp = entry_idx + hold_bars
    return exp if exp < len(rows) else None
...
def simulate_trade(
    ...
    exp_idx = expiry_index(rows, entry_idx, int(config["hold_bars"]))
    if exp_idx is None:
    ...
    sigma = max(state["volatility"].get("annual_vol_used") or 0.20, 0.01)
    ...
    synthetic_raw_credit = spread_mark(spot, short_strike, long_strike, side, years, sigma, risk_free_rate)
    ...
    if credit < float(config.get("min_credit", 0.0)):
        return rejection(
            "credit_below_min",
            entry,
            expiry,
    ...
    option_type = "call" if side == "CCS" else "put"
    short_delta = black_scholes_delta(spot, short_strike, years, sigma, option_type, risk_free_rate)
    ...
    contracts = int((capital * float(config["risk_per_trade_pct"])) // risk_per_contract)
    ...
    for j in range(entry_idx + 1, exp_idx + 1):
        row = rows[j]
        touched_short = touched_short or (
        ...
        mark = spread_mark(row["close"], short_strike, long_strike, side, rem_years, sigma, risk_free_rate)
        if bool(config.get("breach_stop")) and touched_short:
            exit_idx = j
            exit_reason = "short_strike_breach_stop"
            exit_mark = min(mark, width)
            break
        if mark <= credit * (1.0 - float(config["profit_take_pct"])):
            exit_idx = j
            exit_reason = f"profit_take_{int(float(config['profit_take_pct']) * 100)}pct"
            exit_mark = mark
            break
        if mark >= credit * float(config["stop_loss_multiple"]):
            exit_idx = j
    ...
    return {
        ...
        "weather": classify_weather(entry, rows[entry_idx - 1] if entry_idx else None),
...
def simulate_iron_condor_trade(
    ...
    years = option_years(entry["date"], expiry["date"])
    sigma = max(state["volatility"].get("annual_vol_used") or 0.20, 0.01)
    put_hvn = strike_hvn_metrics(state, spot, put_short, int(config["hold_bars"]), sigma)
    call_hvn = strike_hvn_metrics(state, spot, call_short, int(config["hold_bars"]), sigma)
    min_hvn_distance = config.get("min_short_hvn_distance_em") or 0.0
    hvn_distances = [
    ...
    put_raw_synthetic = spread_mark(spot, put_short, put_long, "PCS", years, sigma, risk_free_rate)
    call_raw_synthetic = spread_mark(spot, call_short, call_long, "CCS", years, sigma, risk_free_rate)
    ...
    put_delta = black_scholes_delta(spot, put_short, years, sigma, "put", risk_free_rate)
    call_delta = black_scholes_delta(spot, call_short, years, sigma, "call", risk_free_rate)
    ...
    for j in range(entry_idx + 1, exp_idx + 1):
        row = rows[j]
        touched_put = touched_put or row["low"] <= put_short
        touched_call = touched_call or row["high"] >= call_short
        touched_short = touched_put or touched_call
        rem_years = max(option_years(row["date"], expiry["date"]), 1.0 / 365.0)
        put_mark = spread_mark(row["close"], put_short, put_long, "PCS", rem_years, sigma, risk_free_rate)
        call_mark = spread_mark(row["close"], call_short, call_long, "CCS", rem_years, sigma, risk_free_rate)
        mark = min(put_mark + call_mark, width)
        if bool(config.get("breach_stop")) and touched_short:
            exit_idx = j
            exit_reason = "short_strike_breach_stop"
            exit_mark = mark
    ...
    for i in range(start_idx, len(rows) - max_hold):
        ...
        try:
            ...
            if context_idx < start_idx:
                ...
                continue
            cached = None if signal_lag else (context_cache or {}).get(i)
            if cached and confidence_key in cached["bands"]:
                state = cached["state"]
                band = cached["bands"][confidence_key]
        ...
        for side in sides:
            ...
            trades.append(trade)
            curve.append({"date": trade["exit_date"], "equity": equity})
    summary = summarize_result(trades, curve, cfg, capital, rows[0]["date"], rows[-1]["date"], skipped, rejections)
    return {
        "config": cfg,
        "summary": summary,
...
def summarize_result(trades, curve, cfg, capital, start_date, end_date, skipped=None, rejections=None):
    ...
    return {
        ...
        "pricing_source": (
            "synthetic_only" if trades and trades_with_snapshot_entry == 0 else
...
def score_summary(summary):
    ...
    pf = min(summary["profit_factor"] or 0.0, 5.0)
...
def split_trade_mass_rows(rows, mass_rows, train_fraction=0.70):
    cut = max(80, min(len(rows) - 30, int(len(rows) * train_fraction)))
    test_start = cut - 252 if cut > 252 else 0
    return rows[:cut], mass_rows[:cut], rows[test_start:], mass_rows[test_start:]


def calendar_walk_forward_windows(rows, train_years=3):
    by_year = {}
    for i, row in enumerate(rows):
...
def run_calendar_walk_forward(
    ...
    windows = calendar_walk_forward_windows(rows, train_years=train_years)
    if not windows:
    ...
    for window in windows:
        ...
        cache = build_context_cache(
            ...
            lookback,
            half_life,
            int(configs[0]["hold_bars"]) if configs else DEFAULT_CONFIG["hold_bars"],
            confidences,
        ...
        window_contexts.append((window, context_rows, context_mass_rows, cache))
    for cfg in configs:
        window_rows = []
        for window, context_rows, context_mass_rows, cache in window_contexts:
        ...
        aggregate = summarize_aggregate(cfg["name"], window_rows)
        aggregate.update({
            "side_policy": cfg["side_policy"],
            "confidence": cfg["confidence"],
            "quality_min": cfg["quality_min"],
            ...
            "width": cfg["width"],
            "score": score_summary({
                "trades": aggregate.get("trades", 0),
                "win_rate_pct": aggregate.get("win_rate_pct"),
                "total_return_pct": (aggregate.get("total_pnl_dollars") or 0.0) / capital * 100.0,
                "max_drawdown_pct": aggregate.get("worst_window_drawdown_pct"),
                "profit_factor": aggregate.get("profit_factor"),
                "return_on_total_risk_pct": aggregate.get("return_on_total_risk_pct"),
                "touched_short_rate_pct": aggregate.get("touched_short_rate_pct"),
            }),
        })
        summaries.append(aggregate)
        details.extend(window_rows)
...
def run_sweep(
    ...
    train_rows, train_mass_rows, test_rows, test_mass_rows = split_trade_mass_rows(rows, mass_rows or rows)
    train_results = []
    ...
    train_cache = build_context_cache(
        ...
        lookback, half_life, int(base_config.get("hold_bars", DEFAULT_CONFIG["hold_bars"])), confidences,
...
def write_report(path, payload):
    ...
    lines = [
        ...
        "",
        "Research/model backtest only. Historical option fills are estimated, not observed.",
        "",
    ...
    if rolling.get("available") and rolling.get("summary"):
        lines += [
            "",
            "## Rolling Calendar Walk-Forward",
            "",
            "| Policy | Conf | Quality | Build max | Grav min | Lev max | Width | IC body | IC wing | IC bal | Windows | Trades | Win | P&L | PF | Touch | Worst DD |",
            "|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|",
        ]
        for row in rolling["summary"][:10]:
            ...
            lines.append(
                f"| {row['side_policy']} | {row['confidence']:.2f} | {row['quality_min']:.0f} | "
                f"{row['max_build_up']:.0f} | {(row.get('min_gravity_score') or 0):.0f} | "
                f"{(row.get('max_levitation_score') or 100):.0f} | {row['width']:.0f} | {body * 100.0:.1f}% | ${wing:.2f} | {balance:.2f} | "
                f"{row['windows']} | {row['trades']} | {win} | ${row['total_pnl_dollars']:.2f} | {pf} | {touch} | "
                f"{row['worst_window_drawdown_pct']:.2f}% |"
            )
    lines += [
        "",
        ...
        "- Premiums are Black-Scholes estimates using the entry market-mass annual volatility.",
        ...
        "- Commissions, slippage, profit-taking, and stop-loss rules are included in the model.",
...
def parse_args(argv=None):
    ...
    ap.add_argument("--signal-lag-bars", type=int, default=0,
                    help="Use signals from this many completed bars before entry for lookahead-sensitivity tests.")
...
def load_trade_and_mass_rows(args):
    if args.input_csv:
        ...
        trade_source = f"local CSV {args.input_csv}"
    else:
...
def main(argv=None):
    ...
    payload = {
        ...
        "sources": {
            ...
            "option_snapshots": args.use_option_snapshots,
            "entry_fill_model": args.entry_fill_model,
            "entry_timestamp_policy": args.entry_timestamp_policy,
            "data_grade": (
    ...
    (out_dir / "summary.json").write_text(json.dumps(rounded(payload), indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    trade_fields = [
        "entry_date", "exit_date", "expiry_date", "side", "weather", "regime",
        ...
        "short_delta", "exit_mark_per_share", "credit_risk",
    ...
    (out_dir / "snapshot_replay_rejections.json").write_text(
        json.dumps(rounded(baseline.get("snapshot_rejections") or []), indent=2, ensure_ascii=False) + "\n",
        encoding="utf-8",
    )
    write_csv(
    ...
    (out_dir / "loss_analysis.json").write_text(
        json.dumps(rounded(baseline.get("loss_analysis") or {}), indent=2, ensure_ascii=False) + "\n",
        encoding="utf-8",
    )
    (out_dir / "hvn_outcome_analysis.json").write_text(
        json.dumps(rounded(baseline.get("hvn_outcome_analysis") or []), indent=2, ensure_ascii=False) + "\n",
        encoding="utf-8",
    )
    write_csv(
    ...
    if sweep and sweep["top_test_from_train"]:
        ...
        print(
            f"Best walk-forward candidate: {best['side_policy']} "
